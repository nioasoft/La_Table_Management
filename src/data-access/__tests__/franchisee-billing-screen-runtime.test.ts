...
describe("franchisee billing reopen SQL", () => {
  ...
  it("compares every row with the live source of its own brand using the exact SQL", () => {
    ...
    expect(query.sql).toBe(
      "select \"franchisee_billing\".\"id\", \"franchisee_billing\".\"franchisee_id\", \"franchisee\".\"name\", \"brand\".\"name_he\", \"franchisee_billing\".\"period_year\", \"franchisee_billing\".\"period_month\", \"franchisee_billing\".\"gross_base\", \"franchisee_billing\".\"net_base\", \"franchisee_billing\".\"tier_rate\", \"franchisee_billing\".\"discount_rate_points\", \"franchisee_billing\".\"discount_value\", \"franchisee_billing\".\"royalty\", \"franchisee_billing\".\"marketing\", \"franchisee_billing\".\"subtotal\", \"franchisee_billing\".\"total\", \"franchisee_billing\".\"no_revenue_reason\", coalesce((\n      select sum(\"franchisee_deferral_ledger\".\"amount\")\n      from \"franchisee_deferral_ledger\"\n      where \"franchisee_deferral_ledger\".\"franchisee_id\" = \"franchisee_billing\".\"franchisee_id\"\n    ), 0)::text, \"franchisee_billing\".\"source_file_id\", \"uploaded_file\".\"original_file_name\", (\n      select to_char(\n        max(\"email_log\".\"sent_at\"),\n        'YYYY-MM-DD\"T\"HH24:MI:SS\"Z\"'\n      )\n      from \"email_log\"\n      where \"email_log\".\"entity_type\" = 'franchisee_billing'\n        and \"email_log\".\"entity_id\" = \"franchisee_billing\".\"id\"\n        and \"email_log\".\"status\" = 'sent'\n        and \"email_log\".\"metadata\"->>'messageKind'\n            = 'franchisee_billing_discount_notice'\n    ), coalesce(\"uploaded_file\".\"metadata\"->>'singleBranch', '') <> 'true' and \"franchisee_billing\".\"source_file_id\" is distinct from case \"franchisee\".\"brand_id\" when $1 then $2 when $3 then $4 else null end and not \"franchisee_billing\".\"stale_source_acknowledged\", coalesce(\"uploaded_file\".\"metadata\"->>'singleBranch', '') <> 'true' and \"franchisee_billing\".\"source_file_id\" is distinct from case \"franchisee\".\"brand_id\" when $5 then $6 when $7 then $8 else null end and not \"franchisee_billing\".\"stale_source_acknowledged\", \"franchisee_billing\".\"status\", \"franchisee\".\"owners\" from \"franchisee_billing\" inner join \"franchisee\" on \"franchisee_billing\".\"franchisee_id\" = \"franchisee\".\"id\" inner join \"brand\" on \"franchisee\".\"brand_id\" = \"brand\".\"id\" left join \"uploaded_file\" on \"franchisee_billing\".\"source_file_id\" = \"uploaded_file\".\"id\" where (\"franchisee_billing\".\"period_year\" = $9 and \"franchisee_billing\".\"period_month\" = $10) order by \"franchisee\".\"name\" asc",
    );
