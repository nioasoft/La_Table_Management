...
function logCompletion(
  ...
  console.info(JSON.stringify({
    event: "franchisee_billing_annual_report",
...
function rateLimitResponse(request: NextRequest): NextResponse | null {
  const limit = checkRateLimit(
    `franchisee-billing-annual:${getClientIP(request)}`,
...
async function reportResponse(
  ...
  try {
    const data = await loadFranchiseeBillingAnnualReport(input);
    ...
    console.error("[franchisee-billing-annual] Request failed", {
      requestId: context.requestId,
      error,
    });
...
/** Returns one yearly projection: turnover, royalties or marketing. */
