...
import {
  ...
  removeFromBlacklist,
  isBlacklisted,
} from "@/data-access/bkmvBlacklist";

/**
 * GET /api/bkmvdata/blacklist - Get all blacklisted names
 */
...
/**
 * POST /api/bkmvdata/blacklist - Add a name to the blacklist
...
export async function POST(request: NextRequest) {
  try {
    ...
    const alreadyBlacklisted = await isBlacklisted(trimmedName);
    ...
    // Add to blacklist
    const entry = await addToBlacklist(trimmedName, user.id, notes);
...
/**
 * DELETE /api/bkmvdata/blacklist - Remove a name from the blacklist
