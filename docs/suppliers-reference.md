...
מערכת La Table מנהלת **34 ספקים פעילים** (מתוך 36, למעט 2 ספקי PDF).
...
| דגי הקיבוצים | DAGEI_HAKIBBUTZIM | 8% | percentage | Custom parser: XLSX/ZIP, סכום לפני מע"מ |
...
| ספק | קוד | עמלה | סוג עמלה | הערות |
|-----|-----|------|----------|-------|
| טמפו שיווק | TEMPO | 11% | percentage | מקניות כולל מע״מ |
| תנובה | TNUVA | 6%/7%/5% | percentage | חלב/בשר/בקר (לא חמאה) |
| לאומי קארד | LEUMI_CARD | 0.15% | percentage | שנת כספים אפריל-מרץ, בדיקת חידוש 04/26 |

---

## הגדרות File Mapping לספקים

> **הערה:** כל הסכומים בקבצים הם **לפני מע״מ**
...
- **פרסר מותאם:** `jumon-parser.ts`
- **מבנה קובץ מיוחד:**
  - כל לקוח מופיע בבלוק נפרד (Customer ID בעמודה A רק בשורה הראשונה)
  - עמודה H = סכום העמלה **לכל שורת מוצר** (כבר מחושב בקובץ)
  - הפרסר מסכם את עמודה H עבור כל שורות המוצר של כל לקוח
...
### דגי הקיבוצים (DAGEI_HAKIBBUTZIM)
...
```
- **פרסר מותאם:** `dagei-hakibbutzim-parser.ts`
- **פורמט:** XLSX (גיליון "Report") או ZIP עם מספר קבצי XLSX
- **סטטוס:** רק "מסמך פתוח"
- **סוג מסמך:** "חשבונית מס" + "חשבונית זיכוי" (שאר מסוננים)
- **עמודות:**
  - A: סטטוס מסמך
  - B: סוג מסמך
  - D: תאריך (מספר סידורי Excel)
  - E: מספר עוסק (מזהה זכיין)
  - F: כתובת (שם זכיין להתאמה)
  - O: סכום לפני מע"מ
- **סכומים:** לפני מע"מ (vatIncluded = false)
- **זיכויים:** כבר עם ערך שלילי בעמודה O
- **ZIP:** כל קובץ XLSX = זכיין אחר, מקובצים לפי מספר עוסק

---

### לאומי קארד (LEUMI_CARD)
...
```
- **פרסר מותאם:** `leumi-card-parser.ts`
- **תדירות:** שנתי (שנת כספים אפריל–מרץ, `fiscal_year_start_month = 4`)
- **עמלה:** 0.15% — **אך כבר מגולמת בקובץ**, לא מוכפלת שוב
- **פורמט:** XLSX — טבלת Pivot בגיליון "גיליון1"
- **כותרת:** שורה עם `Row Labels` (עמודת מספר עוסק) + `תחשיב החזר` (עמודת הסכום). הפרסר מאתר את שתי העמודות לפי הכותרות (עמיד להיסט עמודות/שורות)
- **עמודות:**
  - `Row Labels` = מספר עוסק (ח.פ.) — מפתח ההתאמה לזכיין מול `company_id`
  - `תחשיב החזר` = **סכום ההחזר/עמלה הסופי** שלאומי קארד כבר חישב
- **סמנטיקה (אושר ע"י אסי, 2026-06-03):** עמודה F היא ההחזר הסופי, **לא מחזור**. הפרסר מחזיר אותו כ-`preCalculatedCommission` כך ש-`calculateAndCreateCommission` רושם אותו verbatim כ-`commission_amount` בלי להכפיל ב-0.15%.
- **מע"מ:** הסכום סופי — `netAmount = grossAmount = preCalculatedCommission = F`, ללא תוספת 18%
- **שורות מסוננות:** שורת `Grand Total` בסוף; שורות שמתעגלות ל-₪0 (שאריות pivot כגון 0.0032)
- **תאריכים:** הקובץ ללא תאריכי שורה → הפרסר מעלה אנומליית `DATES_NOT_EXTRACTED`, הצוות חייב לאמת שהתקופה שתויגה בהעלאה תואמת (כמו אראל אריזות)
- **התאמת זכיין:** לפי `franchiseeId` (מספר עוסק) → `company_id` (matchType `exact_code`). מספרי עוסק שאינם רשומים יעלו כ-`UNKNOWN_BUSINESS_ID` במודאל הבדיקה עם פעולת "עדכן ח.פ." או התאמה ידנית

### יבולי גורמה (YEVULEI_GOURMET)
...
```
- **פרסר מותאם:** `yevulei-gourmet-parser.ts`
- **עמלה:** 10% — `commission_type=percentage`, `default_commission_rate=10`, **`vat_exempt=true`**
- **שני פורמטים (הפרסר מזהה אוטומטית):**
  - **פורמט A — "דוח מסמכי מכירה" (נוכחי, מ-2026-06):** שורה לכל מסמך/חודש. כותרת: `סה"כ | מע"מ | הנחה | ללא מע"מ | … | שם לקוח | תאריך`. הפרסר מאתר את שורת הכותרת לפי `שם לקוח` + `ללא מע"מ`.
  - **פורמט B — "דוח מכירות" (לגאסי):** Pivot חודשי (לקוחות × חודשים בעברית), שורת ביניים `: סה"כ` לכל לקוח, תאריך מוסק מהחודש האחרון. נשמר לתאימות אחורה.
- **בסיס העמלה (אושר ע"י אסי, 2026-06-03):** **נטו אחרי הנחה, ללא מע"מ = `סה"כ` − `מע"מ`** (עמודת `סה"כ` כבר אחרי הנחה; מחסירים את המע"מ). **לא** עמודת `ללא מע"מ` (היא נטו *לפני* הנחה).
- **מע"מ:** הספק `vat_exempt` → `netAmount = grossAmount`, אין gross-up של 18%.
- **תאריכים (פורמט A):** נקראים מעמודת `תאריך` (Excel serial → `SSF.parse_date_code`, **לא** `cellDates` כדי להימנע מהיסט אזור-זמן ביום).
- **שורות מסוננות:** שתי שורות הסיכום `: סה"כ למסמך` ו-`: סה"כ לדו"ח` מדולגות; שורות שמתעגלות ל-₪0.
- **קבצים חלקיים:** הקובץ עלול להיות מסונן ללקוח אחד (דגימה) → הפרסר **לא** מאמת את סכום השורות מול שורת הסיכום הכוללת.
- **התאמת זכיין:** לפי שם (aliases), כולל הסרת קוד-לקוח פנימי בסוף השם (למשל `1238`). קוד הלקוח הוא פנימי לספק, **לא** ח.פ.
...
| תאריך | שינוי |
|-------|-------|
| 2026-06-03 | יבולי גורמה שינו פורמט ל"דוח מסמכי מכירה" (שורה למסמך) — `yevulei-gourmet-parser.ts` זיהוי דו-פורמט, בסיס עמלה = `סה"כ`−`מע"מ` (נטו אחרי הנחה ללא מע"מ) |
| 2026-06-03 | הוספת פרסר מותאם ללאומי קארד (`leumi-card-parser.ts`) — Pivot, F=החזר סופי (preCalculatedCommission), התאמה לפי ח.פ. |
| 2026-02-05 | העברת לאומי קארד משנתי-חצי לשנתי (שנת כספים אפריל-מרץ) |
